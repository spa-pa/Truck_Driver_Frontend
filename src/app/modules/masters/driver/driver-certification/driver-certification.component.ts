...
export class DriverCertificationComponent implements OnInit, OnDestroy {
  @ViewChild(QRDisplayComponent) qrDisplay!: QRDisplayComponent;
  ...
  private updateQRCode(): void {
    ...
    this.qrConfig = {
      ...this.qrConfig, data: JSON.stringify(qrData)
    };
  ...
  getExpiryProgress(): number {
    ...
    // Derived from this certificate's own issue/expiry dates, rather than a
    // hardcoded 180 (6 months) — so the progress bar stays correct even if
    // the Expiry Config master's `expiry_month` value changes later; each
    // certificate keeps reflecting the validity period it was actually
    // issued with.
    const totalDays = this.getTotalValidityDays();
    const daysLeft =
  ...
  // Total validity window for this certificate, in days: the span between
  // when it was issued (created_at) and when it expires (expiry_date).
  // Falls back to 180 only if either date is missing/invalid.
  private getTotalValidityDays(): number {
    if (!this.certification?.created_at || !this.certification?.expiry_date) {
      return 180;
    }
    const issued = new Date(this.certification.created_at);
    const expiry = new Date(this.certification.expiry_date);
    const diffTime = expiry.getTime() - issued.getTime();
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
  ...
  async downloadPDF(): Promise<void> {
    ...
    for (let i = 0; i < originalCanvases.length; i++) {
      ...
      const destCanvas = clonedCanvases[i] as HTMLCanvasElement | null;
      if (destCanvas) {
      ...
      if (this.qrDisplay) {
        const exportCanvas = await this.qrDisplay.getExportCanvas(220); // match your "medium" desktop size
        const clonedQrCanvas = clone.querySelector(
          ".qr-container canvas",
        ) as HTMLCanvasElement | null;
        if (exportCanvas && clonedQrCanvas) {
          clonedQrCanvas.width = exportCanvas.width;
          clonedQrCanvas.height = exportCanvas.height;
          clonedQrCanvas.getContext("2d")?.drawImage(exportCanvas, 0, 0);
        }
      }
    }
